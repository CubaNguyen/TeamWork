...
const addCategoryController = async (req, res) => {
    try {
        ...
        if (!name) {
            return res.status(400).json({ code: 400, message: "Tên danh mục không được để trống!" });
        }

        let data = await orderService.addCategoryService(name, description);
        return res.status(data.code).json({
            message: data.message,
            code: data.code,
            data: data.data
...
const deleteCategoryController = async (req, res) => {
    try {
        ...
        let data = await orderService.deleteCategoryService(id);
        return res.status(data.code).json({
            message: data.message,
            code: data.code,
            data: data.data
...
module.exports = {
    getAllCategoriesController, addCategoryController, deleteCategoryController
};