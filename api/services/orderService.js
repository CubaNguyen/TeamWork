...
const getAllOrdersService = async () => {
    try {
        ...
        return { message: "Lấy danh sách đơn hàng thành công!", code: 201, data: orders };
    }
    catch (err) {
...
const getOrderDetailService = async (orderId) => {
    try {
        ...
        return { message: "Lấy danh sách đơn hàng thành công!", code: 201, data: order };
    }
    catch (err) {
...
const updateOrderStatusService = async (orderId, newStatus) => {
    try {
        ...
        if (!validStatuses.includes(newStatus)) {
            return { code: 400, message: "Trạng thái đơn hàng không hợp lệ!", data: '' };
        }
...
module.exports = {
    getAllOrdersService, getOrderDetailService, updateOrderStatusService
}