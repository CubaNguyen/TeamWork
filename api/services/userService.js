...
const getProfileService = async (user) => {
    try {
        ...
        return { message: "Lấy profile khách hàng thành công!", code: 201, data: user };
    }
    catch (err) {
...
const getAllUsersService = async () => {
    try {
        ...
        return { message: "Lấy danh sách khách hàng thành công!", code: 201, data: users };
    }
    catch (err) {
...
const detailUserAndHistoryOrderService = async (userId) => {
    try {
        const user = await User.findByPk(userId, {
            include: {
                model: Order,
                as: "orders", // Lấy cả lịch sử đơn hàng
...
module.exports = {
    getProfileService, getAllUsersService, updateUserStatusService, detailUserAndHistoryOrderService, searchUserByNameAndPhoneService
